    ...
    <section class="border-y border-[var(--koku-line)]/55 bg-white py-16 sm:py-20 lg:py-24" x-data="{ open: null }">
        <div class="koku-shell max-w-[82rem]">
            <div class="grid gap-12 lg:grid-cols-[.72fr_1.28fr] lg:gap-20"><div><span class="text-[10px] font-medium uppercase tracking-[.17em] text-[var(--koku-indigo)]">Details, considered</span><h2 class="mt-5 font-serif text-4xl leading-[1.12] tracking-[-.045em]">The character behind the case.</h2><p class="mt-6 text-sm leading-7 text-[var(--koku-muted)]">{{ $product->description ?: 'A considered expression of watchmaking, balancing visual clarity with dependable construction and a quiet presence on the wrist.' }}</p><div class="mt-8 grid grid-cols-3 gap-4 border-t border-[var(--koku-line)] pt-5"><div><span class="block text-[9px] uppercase tracking-[.12em] text-[var(--koku-muted)]">Movement</span><strong class="mt-2 block text-xs">{{ ucfirst($product->movement) }}</strong></div><div><span class="block text-[9px] uppercase tracking-[.12em] text-[var(--koku-muted)]">Style</span><strong class="mt-2 block text-xs">{{ $product->category?->name }}</strong></div><div><span class="block text-[9px] uppercase tracking-[.12em] text-[var(--koku-muted)]">Wearer</span><strong class="mt-2 block text-xs">{{ ucfirst($product->gender) }}</strong></div></div></div>
                <div class="grid gap-3 sm:grid-cols-2">@foreach ($this->specGroups as $groupName => $fields)<div class="h-fit overflow-hidden rounded-2xl bg-[#f6f4f0]"><button type="button" @click="open = open === '{{ Str::slug($groupName) }}' ? null : '{{ Str::slug($groupName) }}'" class="flex w-full items-center justify-between px-5 py-5 text-left"><span class="font-serif text-lg">{{ $groupName }}</span><span class="flex size-8 items-center justify-center rounded-full bg-white text-sm text-[var(--koku-indigo)] shadow-sm" x-text="open === '{{ Str::slug($groupName) }}' ? '−' : '+'"></span></button><div x-show="open === '{{ Str::slug($groupName) }}'" x-collapse><div class="grid gap-4 border-t border-white px-5 py-5">@foreach ($fields as $label => $value)@if(filled($value))<div class="flex items-start justify-between gap-4"><span class="text-[10px] uppercase tracking-[.1em] text-[var(--koku-muted)]">{{ is_string($label) ? $label : 'Detail' }}</span><span class="max-w-[55%] text-right text-xs leading-5">{{ $value }}</span></div>@endif @endforeach</div></div></div>@endforeach</div>
            </div>
        </div>
    </section>
        ...
        <div class="koku-shell max-w-[82rem]">
            <div class="grid gap-8 lg:grid-cols-[22rem_minmax(0,1fr)] lg:gap-12">
                ...
                <div>
                    ...
                    @auth
                        @if($this->canReview)
                            @if($this->ownReview && !$reviewEditing)
                                <article class="mt-6 overflow-hidden rounded-3xl border border-[var(--koku-line)]/65 bg-white shadow-[0_16px_45px_rgba(31,38,53,.06)]"><div class="flex flex-col justify-between gap-5 bg-[#faf9f7] p-5 sm:flex-row sm:items-center sm:p-7"><div><div class="flex items-center gap-3"><p class="text-sm font-medium">Your review</p><span class="rounded-full px-3 py-1.5 text-[9px] font-medium uppercase tracking-[.11em] {{ $this->ownReview->status === 'approved' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700' }}">{{ $this->ownReview->status === 'approved' ? 'Published' : 'Awaiting approval' }}</span></div><div class="mt-3 flex gap-0.5 text-lg text-[#b99872]">@for($star=1;$star<=5;$star++)<span>{{ $this->ownReview->rating >= $star ? '★' : '☆' }}</span>@endfor</div></div><button type="button" wire:click="editReview" class="rounded-xl bg-[var(--koku-indigo)] px-5 py-3 text-xs font-medium text-white shadow-lg shadow-[var(--koku-indigo)]/15">Edit review</button></div>@if($this->ownReview->comment)<p class="px-5 py-5 text-sm leading-7 text-[var(--koku-muted)] sm:px-7">{{ $this->ownReview->comment }}</p>@endif @if($this->ownReview->images->isNotEmpty())<div class="grid grid-cols-3 gap-3 px-5 pb-6 sm:px-7">@foreach($this->ownReview->images as $image)<img src="{{ Storage::url($image->image_path) }}" alt="Your review photo" class="aspect-square w-full rounded-2xl object-cover">@endforeach</div>@endif</article>
                            @else
                    ...
                    @if($approvedReviews->isEmpty())<div class="flex min-h-72 flex-col items-center justify-center text-center"><span class="flex size-14 items-center justify-center rounded-2xl bg-[#f4f2ee] text-xl text-[#b99872]">☆</span><h3 class="mt-5 font-serif text-2xl">Be the first to share.</h3><p class="mt-2 text-sm text-[var(--koku-muted)]">No owner reviews have been published yet.</p></div>@else<div class="mt-8 space-y-5">@foreach($approvedReviews as $review)<article class="rounded-3xl border border-[var(--koku-line)]/65 p-5 sm:p-7"><div class="flex items-start justify-between gap-4"><div class="flex items-center gap-3">@if($review->user->avatar)<img src="{{ Storage::url($review->user->avatar) }}" alt="{{ $review->user->name }}" class="size-11 rounded-xl object-cover">@else<span class="flex size-11 items-center justify-center rounded-xl bg-[var(--koku-indigo)] text-sm font-medium text-white">{{ Str::upper(Str::substr($review->user->name,0,1)) }}</span>@endif<div><p class="text-sm font-medium">{{ $review->user->name }}</p><p class="mt-1 text-[10px] text-[var(--koku-muted)]">{{ $review->created_at->format('M j, Y') }}</p></div></div><span class="rounded-full bg-emerald-50 px-3 py-1.5 text-[9px] font-medium uppercase tracking-[.1em] text-emerald-700">Verified purchase</span></div><div class="mt-5 flex gap-0.5 text-sm text-[#b99872]">@for($star=1;$star<=5;$star++)<span>{{ $review->rating >= $star ? '★' : '☆' }}</span>@endfor</div>@if($review->comment)<p class="mt-4 text-sm leading-7 text-[var(--koku-muted)]">{{ $review->comment }}</p>@endif @if($review->images->isNotEmpty())<div class="mt-5 grid grid-cols-3 gap-3">@foreach($review->images as $image)<img src="{{ Storage::url($image->image_path) }}" alt="Photo shared with review" loading="lazy" class="aspect-square w-full rounded-2xl object-cover">@endforeach</div>@endif</article>@endforeach</div>@endif
                </div>
            </div>
        </div>
    </section>
