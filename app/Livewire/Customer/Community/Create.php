...
class Create extends Component {
    ...
    public function save(): void {
        ...
        $post=CommunityPost::create(['user_id'=>Auth::id(),'product_id'=>$data['productId'],'order_item_id'=>$item->id,'caption'=>trim($data['caption'])?:null,'location'=>trim($data['location'])?:null,'status'=>'pending','visibility'=>'public']);
        foreach($this->photos as $i=>$photo){$size=@getimagesize($photo->getRealPath());$post->media()->create(['file_path'=>$photo->store('community','public'),'width'=>$size[0]??null,'height'=>$size[1]??null,'sort_order'=>$i,'status'=>'pending']);}
        session()->flash('community-success','Your wrist story was sent for approval.'); $this->redirectRoute('community.index');
    }
